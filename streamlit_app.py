...
BASE_PATH = "C:/SmartTraffic_Project"
DATA_PATH = f"{BASE_PATH}/data/processed/accidents_clean.csv"
RAW_DATA_PATH = f"{BASE_PATH}/data/raw/accident_prediction_india.csv"
MODELS_PATH = f"{BASE_PATH}/models"
FEATURE_NAMES_PATH = f"{MODELS_PATH}/feature_names.pkl"
LABEL_ENCODER_PATH = f"{MODELS_PATH}/label_encoder.pkl"
...
def load_data():
    try:
        ...
        try:
            ...
            st.warning("⚠️ Processed data not found. Loading raw data...")
            df = pd.read_csv(RAW_DATA_PATH)
            return df
        except FileNotFoundError:
            st.error("❌ Dataset not found! Please check the file paths.")
            return pd.DataFrame()
...
# --- ANALYTICS DASHBOARD PAGE ---
    ...
    with tab2:
        ...
        if 'Vehicle Type Involved' in df.columns and 'hour' in df.columns:
            ...
            # Heatmap for vehicle type vs hour
